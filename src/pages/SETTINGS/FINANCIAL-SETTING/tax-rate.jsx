...
const TaxRates = () => {
    ...
    try {
      setTimeout(() => {
        if (editingTaxRate) {
          setTaxRates(prev => 
            prev.map(rate => 
              rate.id === editingTaxRate.id ? { ...formData, id: editingTaxRate.id } : rate
            )
          );
  ...
  const modalTitle = editingTaxRate ? "Edit Tax Rate" : "Add Tax Rate";
                    ...
                    {filteredTaxRates.length === 0 && (
                      <tr>
                        <td colSpan={4} className="px-6 py-12 text-center text-gray-500">
                          ...
                          <p className="text-lg">No tax rates found</p>
                          <p className="text-sm">
                            {searchTerm ? 'Try adjusting your search terms.' : 'Add your first tax rate to get started.'}
                          </p>
                        </td>
                      </tr>
                    )}
                  ...
                  <form onSubmit={(e) => { e.preventDefault(); handleSave(); }} className="space-y-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        step="0.01"
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent"
                        placeholder="Enter tax rate (e.g., 18.5)"
                      />
                    </div>
                      ...
                      >
                        {isSaving ? (
                          ...
                          <>
                            ...
                            <span>{editingTaxRate ? "Update" : "Add"} Tax Rate</span>
                          </>
                        )}
                      </button>
                    </div>
