...
function SplashCursor({
  ...
  DENSITY_DISSIPATION = 2.0, // Adjusted for smoother decay
  VELOCITY_DISSIPATION = 1.5, // Adjusted for smoother velocity
  PRESSURE = 0.05, // Reduced for less computation
  PRESSURE_ITERATIONS = 10, // Reduced from 20
  CURL = 2, // Slightly reduced
  SPLAT_RADIUS = 0.15, // Smaller splats
  SPLAT_FORCE = 4000, // Reduced force
  SHADING = true,
  COLOR_UPDATE_SPEED = 8, // Slower color updates
  ...
  const throttleInterval = 16; // ~60fps throttle
  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvas) {
      ...
      let formatRGBA = getSupportedFormat(gl, isWebGL2 ? gl.RGBA16F : gl.RGBA, isWebGL2 ? gl.RGBA : gl.RGBA, halfFloatTexType);
    ...
    const handleMouseMove = e => {
      const now = Date.now();
      if (now - lastMouseMove.current < throttleInterval) return;
